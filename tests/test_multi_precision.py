...
@given(
    ...
    y=st.integers(min_value=0,  max_value=(2**64)),
    base = st.integers(min_value=0, max_value=(5))
)
@settings(deadline=None)
@pytest.mark.asyncio
async def test_ge(multi_precision_factory, x, y, base):

    x_base = x * 2 ** (64 * base)
    y_base = y * 2 ** (64 * base)

    contract = multi_precision_factory
    execution_info = await contract.ge(split(x_base), split(y_base)).call()
...
@settings(deadline=None)
@pytest.mark.asyncio
async def test_gt(multi_precision_factory, x, y, base):

    x_base = x * 2 ** (64 * base)
    y_base = y * 2 ** (64 * base)

    contract = multi_precision_factory
    execution_info = await contract.gt(split(x_base), split(y_base)).call()
...
async def test_div(multi_precision_factory, x, y, base):
    ...
    execution_info = await contract.div(split(x_base), split(y_base)).call()

    q = pack(execution_info.result[0])
    r = pack(execution_info.result[1])   
    assert q == x // y
    assert r == x_base % y_base
